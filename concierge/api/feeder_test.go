...
func TestGetTitleHeader(t *testing.T){
	h1 := getTitleHash("A-Tile")
...
func TestGetFile(t *testing.T){
	...
	if err != nil{
		t.Error("Error while retrieving the document", err)
	}
	if doc != rDoc{
		t.Error(doc, "!=", rDoc)
	}
...
func TestIndexProcessor(t *testing.T){
	...
	for i, tc := range []string{"golang", "programming", "rocks"}{
